...
>
  <template #bottom>
    ...
    <van-button v-if="item.state == 0 && item.orderState == 201" size="mini" :to="tobuy(item.id)" @click="btn(item)"> 一键转卖</van-button>
    ...
    <van-button v-if="item.orderState == 202 || item.orderState == 203" class="gray" size="mini"> {{item.orderState == 202 ?'退款中':'已退款'}}</van-button>  
  </template>
</van-card>
</van-panel>
</template>
